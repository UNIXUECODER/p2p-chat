...
├── core-discovery/                   # M6f: DiscoveryRecordV2 — record shape, Ed25519 signing/verification, peer-ID derivation. Depends only on core-model. Not the full DiscoveryService orchestration interface sketched in §5 — see that section for what's still open
...
// core-discovery (module scaffolded in M6f, but only the record shape + signing/verification
// so far — DiscoveryRecordCodec, Ed25519RecordKeys, SignedDiscoveryRecord. This orchestration
// interface itself (the actual findPeer/announce composition of core-discovery's record codec
// with core-network's DiscoveryController, plus a bootstrap-node list) is still just the
// original sketch, not yet implemented — that's deferred to M6g/M6h, once there's a live daemon
// loop for "announce on a schedule" to actually mean something; see the M6f section of
// README.md for exactly what's implemented today and why the rest was left for then)
...
18. **M6** 🔄 — `node-daemon` composition root + local JSON-RPC/WebSocket API (§7), 1:1 scope. First long-running process holding multiple simultaneous peer sessions — everything through M5e is one-shot CLI demos. Also where M3c's discovery replaces manually hand-carrying a bundle file/multiaddr between terminals. In progress, broken into sub-milestones (M6a–M6h) rather than done as one pass, following the same discipline M2–M5 used:
    ...
    - **M6c** ✅ (fully confirmed — zero external dependency of any kind, nothing hand-traced or stub-compiled for the first time in M6) — Hand-rolled JSON value model (`JsonValue`/`JsonObject`/`JsonArray`/`JsonString`/`JsonNumber`/`JsonBoolean`/`JsonNull`, `JsonCodec`, `node-daemon`). Deliberately scoped to the generic JSON layer only, not JSON-RPC envelope types (M6g's job, once the error-vocabulary decision below is actually settled). One real bug caught by running the insertion-order test: a first-draft defensive copy via `Map.copyOf(...)` doesn't guarantee preserving iteration order (confirmed via its own documentation), silently undoing the reason `LinkedHashMap` was chosen at all. 36/36 real, executed checks after the fix, including a 2^53+1 long surviving exact round-trip (the actual reason numbers are stored as validated raw text, not `double`) and a real supplementary-plane character (U+1F600) proving surrogate-pair handling. See the M6c section of README.md.
    - **M6d** ✅ (fully confirmed — real build, real dependency fix, real fresh test run) — WebSocket transport (`DaemonWebSocketServer`, `DaemonWebSocketFrameHandler`, `WebSocketSession`, `WebSocketTextHandler`, `node-daemon`), on Netty's `WebSocketServerProtocolHandler` — a deliberate decision revisited before building, not a default, given `jvm-libp2p` already pulls Netty transitively (confirmed via the real resolved dependency tree). Researched against Netty's actual tagged source before writing anything, not memory: caught a real stale-API detail this way (`TextWebSocketFrame.text()`, not the older `.getText()`), confirmed the real pipeline order matches Netty's own official example, and confirmed `allowExtensions = false` is correct given permessage-deflate was already out of scope. `node-daemon/build.gradle.kts` had no Netty dependency at all — `:node-daemon:compileJava` failed until `implementation("io.netty:netty-codec-http:4.2.10.Final")` was added; that one line was sufficient on its own (Gradle's `implementation`/`api` distinction is per-hop, not transitively opaque — whatever `netty-codec-http` itself exposes as `api` in its own metadata becomes visible regardless of `node-daemon`'s own scope choice). All 4 `DaemonWebSocketFrameHandlerTest` scenarios, built against Netty's own `EmbeddedChannel` testing utility and scoped to this project's own glue logic rather than re-proving Netty's handshake correctness, passed on a real, fresh (`--rerun-tasks`) run. See the M6d section of README.md.
    - **M6e-2** 🔄 (the testable seam fully confirmed by real execution; the network+crypto wiring stub-compiled against confirmed real signatures, pending a real build) — `SessionManager` (`node-daemon`), the actual daemon core M6a/M6b/M6e-1 were built to be wired into: one listener, any number of concurrent, isolated peer sessions, replacing M5c–M5e's one-shot single-peer demo shape. Resolves two items from "Open M6 design decisions" above (canonical identity, storage transaction boundaries); explicitly defers three more, named rather than silent (relay-delivered inbound reception, full file-transfer chunk handling, pre-key lifecycle policy). Caught a real mistake before it shipped — device id `1`, not `0` — by checking every real `SignalProtocolAddress` construction site in this project (20+) rather than reasoning by analogy to an unrelated storage-layer type. `handleDecryptedPlaintext` (dispatch/dedup/persistence) is real, executed: 4/4 against a real SQLite database. Two new demo Mains, `SessionManagerListenerMain`/`SessionManagerSenderMain`, prove the actual new multi-session capability — the same listener process, unmodified, run against by the sender as many times as desired. See the M6e-2 section of README.md. (Concrete gap surfaced during the user's own multi-sender testing of this milestone, not by review: `SessionManagerListenerMain` publishes one pre-key bundle to a static file once, at startup, and never regenerates it — a later sender reads the same already-consumed one-time prekey an earlier sender's handshake already used. Real, correct rejection either way; not an adversarial-replay finding about `SessionManager` itself. Named as the concrete instance of the pre-key-lifecycle deferral above, and explicitly carried forward into M6f below rather than left implicit.)
    - **M6f** ✅ (the record shape, signing, and verification logic — the actual security-critical part of this milestone — fully confirmed: 19/19 real, executed checks run directly against the real production source, not hand-traced or stub-compiled, a first for anything touching peer identity in this project's M6 work; the two new demo Mains that touch jvm-libp2p itself are stub-compiled against confirmed real signatures only, same constraint as the rest of M6) — `core-discovery` (new module): `DiscoveryRecordV2` — addresses, an optional pre-key bundle, an optional relay preference, and an expiry, all covered by a single Ed25519 signature (`DiscoveryRecordCodec`, `SignedDiscoveryRecord`, `DiscoveryRecord`). The actual MITM defense this milestone exists for — a compromised or malicious relay substituting a different pre-key bundle into a lookup response — is checked directly, not just asserted: `Ed25519RecordKeys` derives the libp2p peer ID straight from a raw Ed25519 public key with no jvm-libp2p dependency at all, verified against the *official* libp2p peer-id spec's own published Ed25519 test vector, cross-checked with an independently written Python implementation, and round-tripped through 5 real JDK-generated keypairs including full sign/verify — closing exactly the gap `PeerId`'s own Javadoc had flagged as unsafe to attempt without the real library. `DiscoveryRegistry` (`relay-server`) gained a best-effort expiry peek — deliberately not a signature check, since the relay was never this system's trust boundary; see that class's own Javadoc for why. Two new demo Mains, `PublishSignedRecordMain`/`LookupSignedRecordMain`, alongside (not replacing) M3c's unsigned-record demos. Explicit scope boundary, named rather than silently dropped: this does not solve pre-key bundle refresh cadence — the concrete staleness bug M6e-2 testing hit needs a live daemon loop to hang a republish policy off of, which doesn't exist until M6g/M6h; what this milestone does establish is that the record format itself doesn't block that fix, since `publish()` is a plain overwrite and republishing is just calling this same code again. See the M6f section of README.md.
    - Remaining: M6g (JSON-RPC method surface + push events, built on M6c and M6d), M6h (`DaemonMain` composition root — also where M6e-2's own deferred items, including pre-key bundle refresh cadence, get picked back up).
