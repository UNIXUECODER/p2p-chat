...
18. **M6** 🔄 — `node-daemon` composition root + local JSON-RPC/WebSocket API (§7), 1:1 scope. First long-running process holding multiple simultaneous peer sessions — everything through M5e is one-shot CLI demos. Also where M3c's discovery replaces manually hand-carrying a bundle file/multiaddr between terminals. In progress, broken into sub-milestones (M6a–M6h) rather than done as one pass, following the same discipline M2–M5 used:
    ...
    - **M6d** ✅ (fully confirmed — verified against real Netty on compile & runtime test classpath) — WebSocket transport (`DaemonWebSocketServer`, `DaemonWebSocketFrameHandler`, `WebSocketSession`, `WebSocketTextHandler`, `node-daemon`), on Netty's `WebSocketServerProtocolHandler` — a deliberate decision revisited before building, not a default, given `jvm-libp2p` already pulls Netty transitively (confirmed via the real resolved dependency tree, and added `io.netty:netty-codec-http:4.2.10.Final` directly to `node-daemon/build.gradle.kts`). Researched against Netty's actual tagged source before writing anything, not memory: caught a real stale-API detail this way (`TextWebSocketFrame.text()`, not the older `.getText()`), confirmed the real pipeline order matches Netty's own official example, and confirmed `allowExtensions = false` is correct given permessage-deflate was already out of scope. 4/4 real JUnit tests in `DaemonWebSocketFrameHandlerTest` executed and passing live against Netty's `EmbeddedChannel` utility. See the M6d section of README.md.
    - Remaining: M6e-2 (wiring the M6e-1 store and M6b's send path into a live multi-session daemon core), M6f (signed discovery records), M6g (JSON-RPC method surface + push events, built on M6c and M6d), M6h (`DaemonMain` composition root).
