...
flowchart TD
    ...
    M6g4["M6g-4: JSON-RPC Method Surface<br/>(JsonRpcRouter + Push Events + DaemonErrorCode)"] --> M6g5
    M6g5["M6g-5: Pre-M6h Hardening Pass<br/>(LICENSE, key permissions, path/size validation, WS auth)"] --> M6g6
    M6g6["M6g-6: Track B - Protocol Versioning<br/>(version byte, capability handshake, compatibility ADR)"] --> M6h
...
* **Goal**: Close the security/process gaps found by an independent peer audit (`docs/pre-m6h-hardening-plan.md`) before `DaemonMain` exists to build on top of them — specifically because one of the four findings (relay-session handling, tracked as that audit's own Track A, still ahead of M6h) reshapes what a composition root would need to own, so fixing what's fixable first is cheaper now than after.
* **Scope**: Phase 0 (hygiene) + Phase 1 (the audit's four 🔴 findings) of that plan. Track A (relay as a real persistent transport) remains, ahead of M6h; Track B is M6g-6, immediately below.
* **New/changed code**:
  * **D-1/D-2/D-3**: Verbatim AGPL-3.0 `LICENSE`. CI (`ci.yml`) runs on every branch/PR instead of only `main`/`master`/`develop`, plus JaCoCo coverage reporting per subproject. `docs/verification-vocabulary.md` (Verified (hardware) / Tested / Compiles / Reviewed) — introduced after finding a real contradiction between this project's own README and `M6g-gap-analysis-and-plan.md §5` about whether M6g-4 had actually been executed (it had; the gap-analysis line was stale).
  * **C-2**: `identity.key`, the Signal identity keypair, and `p2p-chat.sqlite` now written with owner-only (`rw-------`) permissions applied atomically at file-creation time (`JavaIdentityService`, `SignalIdentityVault`, `SqliteDatabase`) — not a `chmod` afterward. WAL/journal sidecar files are a documented residual gap, tracked as an M6h launch-script (`umask 077`) item.
  * **C-4**: `PrintingDaemonEventListener`'s filename sanitizer replaced — allowlist (`[A-Za-z0-9._ ()-]`, leading dots stripped, Windows-reserved names rejected) plus a containment assertion on the fully resolved path, which is the actual load-bearing check. Also closes an equivalent gap in `transferId` (peer-controlled, previously unsanitized) found while implementing this.
  * **C-3**: `FileOfferPayload` validation added to `DefaultFileTransferHandler.onFileOffer` — `chunkSize` bounded to `FileChunker.MIN_CHUNK_SIZE_BYTES`–`MAX_CHUNK_SIZE_BYTES` (1 KiB–8 MiB), `totalChunks > 0`, `fileSize` under a new configurable `maxAcceptedFileSizeBytes` (2 GiB default), and `totalChunks == ceil(fileSize/chunkSize)` consistency. `onFileChunk` rejects an out-of-range `chunkIndex` before the `seek` and a decrypted plaintext longer than `chunkSize`. Symmetric fix applied to `onFileChunkRequest`/`OutgoingTransfer` on the sending side (not named in the audit, found while implementing this).
  * **C-1**: `DaemonWebSocketServer` now binds `127.0.0.1` explicitly (was binding every interface). New `HandshakeAuthHandler` rejects a handshake with a disallowed `Origin` or a missing/incorrect token; new `RpcAuthToken` generates a fresh random token per daemon start, written owner-only, required as a `?token=` query parameter. Required enabling Netty's `checkStartsWith` flag on `WebSocketServerProtocolHandler` — confirmed against the actual bundled `4.2.10.Final` source before writing anything, since the default exact-path match would have silently broken the handshake once a query string was added.
* **Verification**: Real, executed `./gradlew test --rerun-tasks` — **368/368 tasks, 0 failures, 0 errors** (40 new tests over the prior 328-test baseline, arithmetic confirmed exactly). Two POSIX-permission tests (`RpcAuthTokenTest`, `SqliteDatabaseTest`) correctly skipped on the Windows machine this ran on, per their own `Assumptions.assumeTrue` guard. One real bug found by this run and fixed: AssertJ's `PathAssert.startsWith(Path)` calls `toRealPath()` internally, which threw `NoSuchFileException` against `PrintingDaemonEventListenerTest`'s not-yet-created destination paths — a test-assertion-API quirk, not a bug in the production containment logic itself, fixed by asserting the plain `Path#startsWith(Path)` boolean instead. **Not yet covered**: `HandshakeAuthHandlerTest` confirms `HandshakeAuthHandler`'s own logic via an isolated `EmbeddedChannel`, not `DaemonWebSocketServer.start()`'s full real pipeline end-to-end (including `checkStartsWith`) against an actual client — that needs `DaemonMain` to exist.

##### M6g-6 — Track B: Protocol Versioning 🧪 (Tested — local run)
* **Goal**: Per the hardening audit's own sequencing — done ahead of Track A specifically so the new relay frame types that track adds are versioned from birth, not retrofitted.
* **Scope**: B-1, B-2, B-3 of `docs/pre-m6h-hardening-plan.md`. Track A remains, next, before M6h.
* **New/changed code**:
  * **B-1**: A format version byte added to all four hand-rolled codecs (`ChatMessageCodec`, `FileTransferMessageCodec`, `RelayFrameCodec`, `DiscoveryFrameCodec` — `DiscoveryRecordCodec`'s own `FORMAT_MARKER` was already correct, the precedent the other four now follow). New `UnsupportedProtocolVersionException` (duplicated per-module) extends `IllegalArgumentException`. Wire shape kept as `[marker][version][...]`, not `[version][marker][...]`, confirmed necessary by reading `ApplicationMessageRouter`'s actual `plaintext[0]` peek-based dispatch before choosing the shape. Regression sweep across all six touching test files found and fixed 7 pre-existing hand-built-byte-array tests whose buffers had no room for the new version byte.
  * **B-2**: New `HandshakeMessageCodec` + `HandshakeInitPayload`/`HandshakeResponsePayload` (open-ended `Set<String>` capabilities, not a closed enum) handle `EnvelopeType`'s reserved `HANDSHAKE_INIT`/`HANDSHAKE_RESPONSE` markers (0/1), previously routing errors in `ApplicationMessageRouter`. Wired into `SessionManager`: `sendChatMessage`'s session-establishment moment sends a real `HandshakeInitPayload` (`SUPPORTED_CAPABILITIES = {"file-transfer"}`, honest about current real features); receiving an INIT records capabilities and replies with a RESPONSE; new `peerSupports(PeerId, String)` query API. A `senderAddress` field was added to both payloads before finalizing the codec, after checking that this project's existing reply convention (`ChatMessagePayload.senderAddress()`, used for delivery receipts) requires it — there is no network-layer connection info to reply with otherwise.
  * **B-3**: `docs/adr/0001-wire-protocol-compatibility-policy.md`, this project's first ADR. Three categories (unknown type → ignore+log, unknown field → ignore, newer version → refuse distinguishably). Writing it surfaced that `ApplicationMessageRouter.dispatch()` currently does the opposite of category 1 for `GROUP_OP`/`PRESENCE_PING` — throws rather than ignoring — recorded as a tracked follow-up in the ADR itself, deliberately not fixed in this pass.
* **Verification**: Real, executed `./gradlew test --rerun-tasks` — **383/383 tests passing, 0 failures, 0 errors** across all modules (15 new tests over the 368-test M6g-5 baseline). Every assertion in the 7 fixed pre-existing tests and all new tests (`HandshakeMessageCodecTest`, `ApplicationMessageRouterTest`'s new `HandshakeMarkers` cases, two new `SessionManagerReceivePipelineTest` cases against real `fakeSessions`/`fakeNetwork`, and version-rejection tests across all 4 codecs) passed cleanly.
