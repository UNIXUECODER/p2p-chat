...
M5a through M5e now cover the clock, the wire format, the live send/receive/persist loop, dedup/receipt state, and a pre-M6 hardening pass — all confirmed on real hardware, not just compiled. **M6 — the daemon + JSON-RPC/WebSocket API** is next, and now underway, broken into sub-milestones (M6a–M6h) following the same discipline M2–M5 used rather than done as one pass. M6a (shared dispatch), M6e-1 (persistent Signal session store — 39/39 tests confirmed on a real, live run against the real jar), M6b (the outbound send path), M6c (the JSON value model, fully confirmed, zero external dependency of any kind), M6d (the WebSocket transport, on Netty — fully confirmed on a real build), M6e-2 (`SessionManager`, the actual daemon core — its testable seam fully confirmed, the network+crypto wiring pending a real build), and M6f (signed discovery records — the record shape, signing, and verification logic fully confirmed by 19/19 real executed checks; the two new demo Mains pending a real build, same limitation as everything jvm-libp2p-touching in M6) are done. Remaining: M6g (the JSON-RPC method surface + push events, built on M6c and M6d, against §7 exactly as written rather than drifting from it), and M6h (`DaemonMain`, the actual composition root — which is also where M6e-2's own deferred items, relay-delivered inbound reception, full file-transfer handling, and pre-key bundle refresh cadence, would get picked back up). Real external/public address discovery also remains fully unbuilt and is M6/M7's problem, not resolved by M5e's LAN-scoped `DialableAddressResolver`.
