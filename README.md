...
## M6g-2 — Peer routing table, invite codes, `ContactService` 🔄 (sandbox-verified by real, executed checks against the actual production source — 52/52 across three harnesses, including a genuine impersonation-attempt rejection using real Ed25519 signatures; pending confirmation via a real `./gradlew test` run, though unlike every other M6-series milestone touching new production code, nothing added here has any dependency on jvm-libp2p or libsignal-client at all)
...
- **M6g-2** 🔄 — Peer routing table (`PeerRoutingTable` + `V004__peer_routes.sql`), invite code format and `InviteCodeCodec`, `ContactService` orchestrating the `contacts.add` flow via signed discovery lookups. Sandbox-verified by 52 real, executed checks; pending a real `./gradlew test` confirmation.
- **M6g-3** — `SessionManager` event emission (`DaemonEventListener` interface), `DefaultFileTransferHandler` consolidating M4a–M4d's proven chunk logic into `SessionManager`, `sendFile()`/`acceptFileTransfer()` on `SessionManager`.
