...
| M6 | `node-daemon` composition root + local JSON-RPC/WebSocket API (§7), 1:1 scope — **M6a, M6e-1, M6b, M6c, M6d verified** | 🔄 In Progress |
...
**Verified against real Netty:** `io.netty:netty-codec-http:4.2.10.Final` added to `node-daemon/build.gradle.kts`. 4/4 JUnit tests in `DaemonWebSocketFrameHandlerTest` passing live using Netty's `EmbeddedChannel` testing utility — verifying handshake completion, session registration, inbound text frame forwarding, pre-handshake message suppression, and channel disconnection cleanup.

---

## Next milestone

M5a through M5e now cover the clock, the wire format, the live send/receive/persist loop, dedup/receipt state, and a pre-M6 hardening pass — all confirmed on real hardware, not just compiled. **M6 — the daemon + JSON-RPC/WebSocket API** is next, and now underway, broken into sub-milestones (M6a–M6h) following the same discipline M2–M5 used rather than done as one pass. M6a (shared dispatch), M6e-1 (persistent Signal session store), M6b (the outbound send path), M6c (the zero-dependency JSON value model), and M6d (the Netty WebSocket transport) are done and verified live across 88 passing tests in `node-daemon`. Remaining: M6e-2 (wiring the M6e-1 store and M6b's send path into a live multi-session daemon core), M6f (signed discovery records — needed before pre-key bundles can safely enter discovery at all), M6g (the JSON-RPC method surface + push events, built on M6c and M6d, against §7 exactly as written rather than drifting from it), and M6h (`DaemonMain`, the actual composition root). Real external/public address discovery also remains fully unbuilt and is M6/M7's problem, not resolved by M5e's LAN-scoped `DialableAddressResolver`.
