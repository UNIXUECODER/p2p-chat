...
**Remaining M6g work has been split into four sub-milestones** after a systematic gap analysis revealed that M6g's original scope ("implement `JsonRpcRouter` and standard methods/events") implicitly assumed backend capabilities that don't exist yet — see [`docs/M6g-gap-analysis-and-plan.md`](docs/M6g-gap-analysis-and-plan.md) for the full analysis and design decisions:

- **M6g-1** — `StorageService` read-side expansion (`listConversations`, `listContacts`, `getConversation`, `getContact`).
- **M6g-2** — Peer routing table (`PeerRoutingTable` + `V004__peer_routes.sql`), invite code format design and `InviteCodeCodec`, `ContactService` orchestrating the `contacts.add` flow via signed discovery lookups.
