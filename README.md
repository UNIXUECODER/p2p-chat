...
- **Shared decrypted-message dispatch.** `core-filetransfer` and `core-messaging` have separate sealed hierarchies and separate codecs — correct for isolated demos, but a single live daemon session needs one dispatch boundary (a shared envelope, or a central decoder switching on marker values), not every handler independently guessing which codec to try first.
...
- **Pre-key bundle lifecycle.** Demo listeners publish a fresh bundle on every restart. M6 needs an explicit policy: when bundles are (re)generated, how many one-time prekeys exist, whether they're rotated, whether used ones are consumed/persisted, what happens across a daemon restart.
...
M5a through M5e now cover the clock, the wire format, the live send/receive/persist loop, dedup/receipt state, and a pre-M6 hardening pass — all confirmed on real hardware, not just compiled. **M6 — the daemon + JSON-RPC/WebSocket API** is next. "Open M6 design decisions" above is the starting agenda, not a surprise list: canonical peer identity, shared chat/file-transfer dispatch, one outbound send path (including finally wiring `ConnectionStrategy`'s proven relay fallback into it, so peers on different networks — not just different machines on one LAN — can actually reach each other), discovery record shape, pre-key bundle lifecycle, storage transaction boundaries, and a daemon-facing error vocabulary. Real external/public address discovery also remains fully unbuilt and is M6/M7's problem, not resolved by M5e's LAN-scoped `DialableAddressResolver`. All of this was deliberately left for M6 rather than retrofitted onto M5's one-shot CLI demos, since a long-running daemon with real configuration is a more natural home for these decisions than a demo `Main` that M6 is going to replace anyway.
