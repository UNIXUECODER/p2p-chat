...
## M6g-3 — `SessionManager` event emission, `DefaultFileTransferHandler`, `sendFile`/`acceptFileTransfer` ✅ (`DefaultFileTransferHandler` and `DaemonEventListener`/`FileTransferHandler` fully confirmed by real, executed checks against the actual production classes — 18/18 across a two-independent-instance end-to-end harness, real AES-256-GCM chunk encryption and SHA-256 whole-file verification throughout, no mocks; the `SessionManager` wiring itself — five new call sites plus two new public methods — is hand-traced against confirmed real signatures, same jvm-libp2p/libsignal-client limitation as every other SessionManager change this project has made)
...
**A genuine, deliberately out-of-scope gap, named rather than silently accepted:** an offer sitting at `OFFERED` — received, not yet accepted — does not survive a daemon restart. The file key and the sender's reply address live only in `DefaultFileTransferHandler`'s in-memory maps, deliberately never persisted in cleartext, the same security posture `FileKey` itself already takes. The identical shape of gap `SessionManager`'s own Javadoc already names for relay-delivered inbound reception. Similarly, `sendFile` does not persist a `file_transfers` row for the sending side at all — the current schema has no column distinguishing a sent transfer from a received one, so a row written here would be indistinguishable from an inbound offer's row; adding that distinction is real schema-design work, not a casual side effect of this method.

**Verified for real wherever the dependency graph allowed it — 18 real, executed checks against a genuine two-instance, no-mocks harness.** `DefaultFileTransferHandler`, `FileTransferHandler`, and `DaemonEventListener` inherited `core-filetransfer`'s (M4a) zero-dependency property once the conversation-ID sharing decision above was reversed, so a standalone harness wired two fully independent `DefaultFileTransferHandler` instances — separate `StorageService`, separate temp directories, simulating two separate daemons — together via a fake transport, and ran: a single-chunk happy path with byte-exact reconstruction; a 137-byte file at a 10-byte chunk size (14 chunks, a genuine short last chunk) reassembling exactly; direct proof the accept gate blocks chunk requests until `acceptFileTransfer` is called (zero requests from an offer alone, exactly one after accepting); a simulated daemon restart (fresh handler instance, same `StorageService`, same partially/fully-written file) correctly requesting zero already-received chunks on re-offer; a deliberately-wrong whole-file hash correctly marked `FAILED` rather than `COMPLETED`; and a duplicate offer after acceptance correctly leaving an already-completed transfer untouched. `SessionManager`'s own five new/changed call sites (the event-firing wiring, `sendFile`, `acceptFileTransfer`) are hand-traced against confirmed real signatures — the same limitation as every prior `SessionManager` change, since `SecureSessionService`/`SignalProtocolAddress` usage elsewhere in that same file makes the whole file uncompilable without `libsignal-client`. The equivalent real JUnit files (`DefaultFileTransferHandlerTest`, three new `SessionManagerReceivePipelineTest` cases for the listener wiring) are included for a real `./gradlew test` confirmation, not yet run at the time this was written. `onNetworkStatusChanged` specifically isn't covered by the new `SessionManagerReceivePipelineTest` cases — its two trigger points both need `FakeSecureSessionServiceForTest` capabilities (`decrypt`/`establishSession`) that fake deliberately doesn't provide, named honestly in that test file rather than forcing a low-value test or silently skipping the gap.
...
**Remaining M6g work has been split into four sub-milestones** after a systematic gap analysis revealed that M6g's original scope ("implement `JsonRpcRouter` and standard methods/events") implicitly assumed backend capabilities that don't exist yet — see [`docs/M6g-gap-analysis-and-plan.md`](docs/M6g-gap-analysis-and-plan.md) for the full analysis and design decisions. M6g-1, M6g-2, and M6g-3 are done:
...
- **M6g-3** ✅ — `SessionManager` event emission (`DaemonEventListener`), `DefaultFileTransferHandler` consolidating M4a–M4d's proven chunk logic into a real, pluggable implementation, `sendFile()`/`acceptFileTransfer()` on `SessionManager`.
- **M6g-4** — JSON-RPC router, method dispatch against §7's original method names, push events (`event.message.received`, `event.transfer.progress`, `event.network.statusChanged`), and `DaemonErrorCode` enum.

Then **M6h** (`DaemonMain`, the composition root + E2E test) picks up the remaining deferred items: relay-delivered inbound reception, pre-key bundle refresh cadence, and the live two-daemon integration test. Real external/public address discovery also remains fully unbuilt and is M7's problem, not resolved by `DialableAddressResolver`'s LAN scope.
