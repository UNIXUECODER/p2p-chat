...
| M6 | `node-daemon` composition root + local JSON-RPC/WebSocket API (§7), 1:1 scope — **M6a & M6e-1 verified** | 🔄 In Progress |
...
## M6a — Shared decrypted-message dispatch ✅ (verified)
...
Pure JDK — `node-daemon`'s dependency on `core-messaging`/`core-filetransfer` was already there for the demo Mains, no new dependency added. Verified with 17 JUnit 5 tests in `ApplicationMessageRouterTest` covering valid chat/file payloads, unknown/reserved markers, truncated byte streams, and codec validation propagation.

---

## M6e-1 — Persistent Signal Protocol session store ✅ (verified against real SQLite & libsignal-client)
...
**Verified with real `libsignal-client` 0.94.0 and SQLite storage:**
- **Exact signature alignment**: `saveIdentity` returns `IdentityChange` (`NEW_OR_UNCHANGED`/`REPLACED_EXISTING`); `markKyberPreKeyUsed` takes `(kyberPreKeyId, signedPreKeyId, ECPublicKey baseKey)` throwing `ReusedBaseKeyException`.
- **Persistent Base-Key Replay Protection (`V003__kyber_base_key_replay.sql`)**: Tracks seen `baseKey.serialize()` bytes in SQLite table `signal_kyber_base_keys_seen` with composite primary key `(kyber_prekey_id, signed_prekey_id, base_key)`. Guarantees replay defense survives full daemon restarts and deduplicates by raw binary byte values rather than Java object memory references.
- **22 passing tests in `SqliteSignalProtocolStoreTest`**: Including 16-thread concurrent load, restart survival across SQLite database closure/reopening from disk, base-key replay rejection across restart boundaries, and a complete end-to-end PQXDH session handshake + Double Ratchet continuation over `WHISPER` frames across a simulated daemon shutdown of both peers.
...
- **One outbound send path.** Chat/file demos all call `sendEnvelope` directly today. M6 should build one abstraction — direct-first attempt, relay fallback via the already-proven `ConnectionStrategy`, timeout, async execution off libp2p callback threads, `DIRECT`/`RELAYED`/`UNREACHABLE` result reporting — once, early, rather than scattering direct calls across JSON-RPC methods.
...
M5a through M5e now cover the clock, the wire format, the live send/receive/persist loop, dedup/receipt state, and a pre-M6 hardening pass — all confirmed on real hardware. **M6 — the daemon + JSON-RPC/WebSocket API** is actively underway: **M6a** (shared dispatch) and **M6e-1** (persistent Signal session store with V003 replay defense) are complete and verified across all test suites.

**Next sub-milestone:** **M6b** (one outbound send path, uniting direct dial + `ConnectionStrategy` relay fallback + async offloading off Netty threads), followed by M6c (hand-rolled JSON model), M6d (RFC 6455 WebSocket transport), M6e-2 (wiring the store into live multi-session manager), M6f (signed discovery records), M6g (JSON-RPC method surface + push events), and M6h (`DaemonMain`).
