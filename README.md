...
| M6 | `node-daemon` composition root + local JSON-RPC/WebSocket API (§7), 1:1 scope — **M6a–M6g-4 done** (dispatch, outbound send, JSON model, WebSocket, Signal store + SessionManager, signed discovery, JSON-RPC router); **M6g-5** (pre-M6h hardening pass, 🧪 tested — real `./gradlew test`, 368/368) and **M6g-6** (Track B: protocol versioning, 🔨 compiled/hand-traced) done; **M6h** (`DaemonMain`) remaining, after Track A from the hardening audit — see [`M6g-gap-analysis-and-plan.md`](docs/M6g-gap-analysis-and-plan.md) and [`pre-m6h-hardening-plan.md`](docs/pre-m6h-hardening-plan.md) | 🔄 In Progress |
...
## M6g-5 — Pre-M6h hardening pass 🧪 Tested (local run) (see [`docs/verification-vocabulary.md`](docs/verification-vocabulary.md)) — real, executed `./gradlew test --rerun-tasks`, 368/368 tasks green, run on a real machine outside the sandbox this was implemented in; NOT yet the two-real-process hardware checkpoint the project's own convention reserves for network/crypto-touching work (see the C-1 note below), which is why this stops short of ✅
...
**Real run: 368/368, 0 failures, 0 errors — and the arithmetic was checked, not just the headline number.** This pass added 40 new tests (1 to `IdentityServiceTest`, 1 to `SignalIdentityVaultTest`, 2 new in `SqliteDatabaseTest`, 17 new in `PrintingDaemonEventListenerTest`, 9 new in `DefaultFileTransferHandlerTest`, 6 new in `HandshakeAuthHandlerTest`, 4 new in `RpcAuthTokenTest`) against a pre-existing baseline of 328 — 328 + 40 = 368 exactly, confirming nothing silently dropped or duplicated between the hand-traced version and the executed one. The two `assumeTrue`-guarded POSIX-permission tests (in `RpcAuthTokenTest` and `SqliteDatabaseTest`) correctly reported as skipped rather than failed, run on Windows — the documented non-POSIX gap working as designed, not a gap in the run.

**One real bug the hand-tracing couldn't have caught, because it lives in a third-party library, not in this codebase.** AssertJ's `PathAssert.startsWith(Path)` canonicalizes both sides via `toRealPath()` before comparing — which throws `NoSuchFileException` for a path that doesn't exist on disk yet, exactly what `PrintingDaemonEventListenerTest`'s containment assertions check (`resolveSafeSavePath` computes *where to save*, before anything is written). Fixed in all 4 affected assertions by comparing via the plain `java.nio.file.Path#startsWith(Path)` boolean method instead, wrapped in `assertThat(boolean).isTrue()`, which does no filesystem access at all. Worth being precise about: this was a test-assertion-API quirk, not a bug in `PrintingDaemonEventListener` itself — the actual containment logic was correct throughout, which the same test run confirms (17/17 pass once the assertion stops touching the filesystem).

---

## M6g-6 — Track B: protocol versioning 🧪 Tested (local run) (see [`docs/verification-vocabulary.md`](docs/verification-vocabulary.md)) — real, executed `./gradlew test --rerun-tasks`, 383/383 tests passing across 30 executed tasks, 0 failures, 0 errors, run on Windows hardware

Per the hardening audit's own sequencing: done ahead of Track A (relay-as-real-transport, next) specifically so the new relay frame types that track adds are versioned from birth, rather than needing a follow-up pass to add versioning to code that didn't exist yet when B-1 landed.

**B-1 — a format version byte in all four hand-rolled codecs** (`ChatMessageCodec`, `FileTransferMessageCodec`, `RelayFrameCodec`, `DiscoveryFrameCodec` — `DiscoveryRecordCodec`'s own `FORMAT_MARKER` was already correct and untouched, the precedent the other four now follow). New `UnsupportedProtocolVersionException` (duplicated per-module, same C-2-style trade-off) extends `IllegalArgumentException` so every existing broad catch site keeps working unchanged, while new code can distinguish "corrupt" from "too new." Wire shape is `[marker][version][...]`, not `[version][marker][...]` — the marker has to stay at index 0 unmoved, since `ApplicationMessageRouter` peeks `plaintext[0]` to decide which codec to even call, before either codec's `decode()` runs; confirmed by reading that router's actual dispatch logic before choosing this shape, not assumed. Regression sweep across all six test files touching these codecs found and fixed 7 existing tests whose hand-built byte arrays had no room for the new version byte (the version-byte read was silently consuming what used to be the first byte of a length-prefix field under test, misreading it as a bogus version) — fixed by inserting a valid version byte rather than weakening the check, plus 2 files (`ApplicationMessageRouterTest`, `SessionManagerReceivePipelineTest`) confirmed to need no changes at all, since they only ever use each codec's real `encode()`/`decode()` pair, never hand-built bytes.

**B-2 — capability negotiation at session start.** `EnvelopeType`'s reserved `HANDSHAKE_INIT`/`HANDSHAKE_RESPONSE` markers (0/1), previously routing errors in `ApplicationMessageRouter`'s own "should never happen" Javadoc, now carry a real `HandshakeMessageCodec` (new `HandshakeInitPayload`/`HandshakeResponsePayload`, an open-ended `Set<String>` of capability names, not a closed enum — a future capability is one string added at the call site, not a wire-format change). Wired into `SessionManager`, not left as an unused codec: `sendChatMessage`'s session-establishment moment now also sends a `HandshakeInitPayload` listing this build's actual, honest capabilities (`{"file-transfer"}` today, not the audit's own illustrative relay-spool/groups/mls, none of which are real yet); receiving an INIT records the sender's capabilities and replies with this build's own RESPONSE; a new `peerSupports(PeerId, String)` query API is the audit's own stated point of B-2 ("what makes M8 additive rather than breaking"). A real design correction made before finalizing: the first draft of both payloads had no `senderAddress` field, until checking how this project already replies to an inbound message (`ChatMessagePayload.senderAddress()`, used for delivery receipts since there's no network-layer connection info to reply with otherwise) showed the same field is required here too — added before writing the codec, not discovered after.

**B-3 — the compatibility policy, written down** (`docs/adr/0001-wire-protocol-compatibility-policy.md`, this project's first ADR). Three categories: unknown message type → ignore + log, unknown field → ignore, newer major version → refuse distinguishably (already implemented by B-1). Written as an honest gap check, not just a policy statement: writing it surfaced that `ApplicationMessageRouter.dispatch()` currently does the *opposite* of category 1 for `GROUP_OP`/`PRESENCE_PING`/any unrecognized marker — it throws. Recorded as a tracked follow-up in the ADR itself, deliberately not fixed in the same pass (a real design decision with its own regression surface, not a one-line fix).

**What's confirmed and what isn't, precisely.** Confirmed via real execution of `./gradlew test --rerun-tasks` — 383/383 tests passing, 0 failures, 0 errors (15 new tests over the 368-test M6g-5 baseline across `HandshakeMessageCodecTest`, `ApplicationMessageRouterTest`'s new `HandshakeMarkers` cases, `SessionManagerReceivePipelineTest`'s capability negotiation tests, and version-rejection tests across all 4 codecs). Arithmetic confirmed exactly: 368 + 15 = 383.
...
- **M6g-5** 🧪 — Pre-M6h hardening pass (see above). All four 🔴 findings from the audit fixed; real, executed `./gradlew test --rerun-tasks` — 368/368, 0 failures, 0 errors, run on real hardware outside the environment this was implemented in. One real bug found by that run (an AssertJ `Path` canonicalization quirk in test code, not a production bug) fixed and folded back in. Not yet the two-real-process hardware checkpoint the project reserves for network-facing work (see the C-1 note above) — that's still ahead, likely alongside or after M6h.
- **M6g-6** 🧪 — Track B: protocol versioning (see above). B-1 (version byte, all four codecs), B-2 (capability negotiation handshake, wired into `SessionManager`), B-3 (compatibility-policy ADR, which itself surfaced a real gap between policy and `ApplicationMessageRouter`'s current behavior — tracked in the ADR, not yet fixed). Real, executed `./gradlew test --rerun-tasks` — 383/383 tests passing across all subprojects, 0 failures, 0 errors.

Then **M6h** (`DaemonMain`, the composition root + E2E test) picks up the remaining deferred items: relay-delivered inbound reception, pre-key bundle refresh cadence, the file-transfer completion receipt and locally-detected-send-failure push notification (both named above), and the live two-daemon-plus-JSON-RPC integration test. Real external/public address discovery also remains fully unbuilt and is M7's problem, not resolved by `DialableAddressResolver`'s LAN scope. Track A (relay as a real persistent transport, from the same hardening audit) remains, deliberately sequenced before M6h itself, now that Track B is done — see `docs/pre-m6h-hardening-plan.md`.
