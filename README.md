...
- **M6g-3** ✅ — `SessionManager` event emission (`DaemonEventListener`), `DefaultFileTransferHandler` consolidating M4a–M4d's proven chunk logic into a real, pluggable implementation, `sendFile()`/`acceptFileTransfer()` on `SessionManager`, full lifecycle state handling (`COMPLETED` deduplication, `FAILED` retry, partial crash resumption), and outgoing transfer TTL eviction.
